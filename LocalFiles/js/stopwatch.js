var state = 0; // Keep track of start/pause state to enable button toggling.

// Toggle start/stop of the counter.
function toggleStartPause() {
    state = (state == 0) ? 1 : 0;

    if (state == 0) {
        mStart();
        myimagebutton.setProperty("image", pauseIcon);
    } else {
        mPause();
        myimagebutton.setProperty("image", startIcon);
    }
}
...
function mStart() {
    ...
    if(lapCounter == null){
        ...
        mosync.nativeui.callJS(mapHandle, "startWatching()");
...
function mPause() {
    ...
    if(lapCounter != null){ 
        ...
        mosync.nativeui.callJS(mapHandle, "stopWatching()");
...
function mReset() {
    ...
    mosync.nativeui.callJS(mapHandle, "clearCoordinates()");
